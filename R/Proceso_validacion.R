...
###########################################################
# Datos correspondientes al periodo 2001_2017
...
for (s in 1:4) {
  ...
  while ((indice_final+prediccion) <= length(vec_temporal)) {
    ...
    # Extraer tambien los limites del intervalo de credibilidad del 95%
    ...
    for(i in 1:size_datos){
      x = inla.rmarginal(size_muestra, marginales_pred[[i]]) # generar muestra aleatoria
      pred_x = x*offset[i]  
  ...
  # Calcular las medidas de validacion del modelo ajustado,
  ...
  # Calcular la media de cada una de las medidas obtenidas, para cada ano predicho a futuro
...
# Calculo de las poblaciones medias para cada region (105)
...
# Seleccionar regiones con tamanos de poblacion diversos
